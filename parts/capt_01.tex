...
First time use of \gls{uti}. Then I can use it again as \gls{uti}, or its plural as \glspl{uti}. \Glspl{maths}, for the begging a phrase, or plural like \glspl{maths}.

If I need only the acronym, then use \acrshort{scuba}. However, is I need only the full name, the use \acrlong{radar}. Finally, we can use notation like \gls{not:vet}.

For symbols, you can use the following idea as
\begin{equation}
    \gls{symb:E}=\frac{\pi}{2}\sum x_{2}d,\label{eq:equatoin1}
\end{equation}
where \gls{symb:E} is the \glsdesc{symb:E} given in \glsunit{symb:E}.

Try using the \equationautorefname\,\eqref{eq:equatoin1}. For acronyms, you can change its description as \gls{dl}.

\global\long\def\teste{\int_{x}\frac{1}{2}\sum x_{2}dt}
...
\import{figs/}{figuraExemplo.pdf_tex}%

\caption{Figura exemplo}
\label{fig:FiguraExemploLabel}
\end{figure*}
...
Maecenas\footnote{Teste de footnote} nec orci at augue sollicitudin egestas. Suspendisse non risus
eget sapien sagittis ullamcorper ut at dolor. Class aptent taciti
sociosqu ad litora torquent per conubia nostra, per inceptos himenaeos.
Quisque a vestibulum augue. Donec erat mauris, molestie sed sagittis
sed, porttitor id elit. Nunc blandit accumsan lacus eu semper. Aliquam
